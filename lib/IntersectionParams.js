        ...
        if ((index + param_length) < tokens.length) {
            ...
            switch (mode) {
                ...
                case "V":
                    ...
                    break;
                case "v":
                    segment = new RelativeVLineto(params, previous);
                    break;
                case "L":
...
function RelativeVLineto(params, previous) {
    if (arguments.length > 0) {
        this.init("v", params, previous);
    }
}
RelativeVLineto.prototype = new RelativePathSegment();
RelativeVLineto.prototype.constructor = RelativeVLineto;
RelativeVLineto.superclass = RelativePathSegment.prototype;

RelativeVLineto.prototype.init = function(command, params, previous) {
    const relativeOffsets = [0, params.pop()];
    RelativeVLineto.superclass.init.call(this, command, relativeOffsets, previous);
};
